...
import { Plus, Trash2, Copy, Check, Info, Palette, RotateCcw, MessageSquare } from 'lucide-react'
...
const TEXT_PRESETS   = [['#ffffff','#ffffff'],['#e2e8f0','#e2e8f0'],['#c8cde8','#c8cde8'],['#a0a0d0','#a0a0d0']]
...
function ThemePanel({ theme, onChange, tc, tch }) {
    ...
    <div style={{ ...S.card, padding:20, marginBottom:16, animation:'fade-up 0.18s ease-out' }}>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))', gap:20 }}>
        ...
        <div>
          ...
          <Swatches value={theme.textSecondary} options={TEXT_PRESETS} onChange={v => set('textSecondary', v)} />
        </div>
        ...
        {/* Chat & Effects */}
        <div>
          <p style={S.sectionLabel}>{tch.chatEffects}</p>
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            <Slider label={tch.maxMessages} value={theme.maxMessages} min={4} max={20} onChange={v => set('maxMessages', v)} />
...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    getOne('chat_theme').then(t => { if (t) setTheme(prev => ({ ...prev, ...t })) })
  }, [])

  useEffect(() => {
    const off = onTableChange('chat_sessions', () => getAll('chat_sessions').then(d => setSessions(d)))
    return off
  }, [])

  const handleThemeChange = async (next) => { setTheme(next); await setOne('chat_theme', next) }

  const createSession = async () => {
    if (!form.name) return
    const data = await insert('chat_sessions', { name: form.name, status: 'active' })
  ...
  const deleteSession = async () => {
    ...
    await remove('chat_sessions', selectedId)
    ...
    <div>
      ...
      {showInfo && (
        <div style={{
          ...
          <MessageSquare size={16} style={{ color:'#d4af37', flexShrink:0, marginTop:1 }} />
          <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
            <span style={{ fontSize:12, fontWeight:600, color:'#d4af37' }}>{tch.title}</span>
      ...
      {showTheme && <ThemePanel theme={theme} onChange={handleThemeChange} tc={tc} tch={tch} />}

      {showNew && (
        <div style={{ ...S.card, padding:20, marginBottom:20, animation:'fade-up 0.2s ease-out' }}>
          <p style={S.label}>{tch.newChat}</p>
          <div style={{ display:'flex', gap:10, flexWrap:'wrap', alignItems:'flex-end' }}>
            <input className="input" style={{ flex:'1 1 200px' }} placeholder={tch.name}
      ...
      {sessions.length === 0 ? (
        <div style={{ textAlign:'center', padding:'60px 0', color:'#4a4842', fontSize:13 }}>{tch.empty}</div>
      ) : (
        <div style={{ ...S.card, border:'1px solid rgba(212,175,55,0.2)', overflow:'visible' }}>
          <div style={{ padding:24 }}>
            <ChatOverlay theme={theme} />
          </div>
