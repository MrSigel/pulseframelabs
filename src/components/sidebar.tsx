        ...
        {items.map((item) => {
          ...
          if (item.comingSoon) {
            return (
              <div
                key={item.href}
                className="relative flex items-center gap-3 rounded-lg px-3 py-2 text-[13px] font-medium opacity-40 cursor-not-allowed select-none"
              >
                <Icon className="h-4 w-4 shrink-0 text-muted-foreground" />
                <span className="text-muted-foreground">{item.name}</span>
                <span className="ml-auto text-[8px] font-bold uppercase tracking-wider text-muted-foreground/70 bg-muted-foreground/10 px-1.5 py-0.5 rounded">
                  Soon
                </span>
              </div>
            );
